...
class GenStrings {
    ...
    func perform(path: String? = nil) {
        ...
        for filePath in allFiles {
            ...
            localizableStrings.append(stringsInFile)
        }
        // We sort the strings)
        var processedStrings = String()
        for (key, value) in localizableStrings {
//            if value.count > 0 {
//                processedStrings.append("// \(key) \n")
//            }
            for string in value {
                if processedStrings.contains("\"\(string)\" : \"<#UNLocalized#>\", \n") == false {
                    processedStrings.append("\"\(string)\" : \"<#UNLocalized#>\", \n")
                }
            }
        }
    ...
    func localizableStringsInFile(filePath: URL) -> (String,[String]){
        do {
            ...
            return ("\(filePath.lastPathComponent)",localizedStringsArray)
        } catch {}
        return ("",[String]())
    }
